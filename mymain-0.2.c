...
unsigned long get_rand(int min,int max);

void sand_priority(void)
{
	int i;
	for(i=0;i<MAX_TASK_NUM;i++)
		task[i].priority=get_rand(0,PRIORITY_MAX);
}
void __init my_start_kernel(void)
{
	...
	//task[pid].priority=pid;/////
	task[pid].priority=get_rand(0,PRIORITY_MAX);
    }
...
{
    int i = 0;
//    printk(KERN_NOTICE "process %d running!!!\n",my_current_task->pid);
    while(1)
    {
        i++;
        if(i%10000000 == 0)
        {
            ...
            {
                my_need_sched = 0;
        	//my_schedule(my_current_task->pid);
		sand_priority();
	   	my_schedule();  
...
unsigned long get_rand(min,max)
{
	unsigned long a;
	unsigned long umin,umax;
	umin=(unsigned long)min;
